...
{pyFoamStudy.studydir.CASE_CSVs}
...
def parse_studydir(args):
    ...
    info = pyFoamStudy.io.read_info(studyinfofile)
...
def append_info(args):
    ...
    pyFoamStudy.io.write_info(info, studyinfofile)
...
def main():
    ...
    print(f"Agglomerate {pyFoamStudy.studydir.CASE_CSVs} of all cases with metadata and studyparameters into {args.database_csv}")
    cmd_str_agglo = f"study_agglomerate-database.py {args.studydir} {args.database_csv}"
    run(cmd_str_agglo, check=True, shell=True)

    append_info(args)

    # refinementparameter = pyFoamStudy.studycsv.get_refinementlabel(pd.read_csv(args.database_csv, header=[0,1]))
